    ...
    def printChat(self):
        if self.WhichScreen == "Social":
            if self.isGlobalChat:
                ...
                chat = self.privateChat[self.lastSelectedFriend][self.chatOffset:13+self.chatOffset]
...
#This code create the customiztion window
    def createCustomization(self):
        ...
        self.myImage = OnscreenImage(parent=render2dp,
                                     image = 'IMAGES/customization_w2.png',
                                     pos = (0, 0, 0),
                                     scale = (1, 1, 1))
    ...
    def handlePrivateChatNotification(self, username, msg):
        if self.privateChat[username] is None:
            self.privateChat[username] = [[username,msg]]
        else:
            self.privateChat[username].insert(0, [username, msg])
